...
#define SCL_H()		HAL_GPIO_WritePin(IIC_SCL_GPIO, IIC_SCL_PIN, GPIO_PIN_SET)
#define SCL_L()		HAL_GPIO_WritePin(IIC_SCL_GPIO, IIC_SCL_PIN, GPIO_PIN_RESET)
#define SDA_H()		HAL_GPIO_WritePin(IIC_SDA_GPIO, IIC_SDA_PIN, GPIO_PIN_SET)
#define SDA_L()		HAL_GPIO_WritePin(IIC_SDA_GPIO, IIC_SDA_PIN, GPIO_PIN_RESET)
        ...
        uint8_t* pWrtData,InvChoose InvSW,ISorNOT isRefresh)
{
  ...
  {
    effBits = (lastY-yPos+1>=8)?8:(lastY-yPos+1);
    for(xPos=x,xCnt=0;xCnt+6<=showW;xCnt+=6)//!!!!修改未测
    {
...
void OLED_Init(void)
{
	...
	OLED_WriteCommand(0xa0);//--Set SEG/Column Mapping     0xa0左右反置 0xa1正常
	OLED_WriteCommand(0xc0);//--Set COM/Row Scan Direction   0xc0上下反置 0xc8正常
